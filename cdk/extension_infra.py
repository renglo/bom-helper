"""Extension-declared AWS resources from installer/infra/cdk_extension.json.

Owned by the stack that hosts the extension (Stack B for hub.python, peer CDK
for peers.<id>.extensions). The platform vector bucket + default KB stay on Stack A.
"""
...
from s3_vectors_kb_access import (
...
def platform_vector_bucket(
    env_name: str, aws_account: str, aws_region: str
) -> tuple[str, str]:
    """Same name/ARN Stack A AiStorage uses — peers do not import Stack A."""
    name = f"{env_name.lower()}-vectors-{aws_account}"[:63].rstrip("-.")
    arn = f"arn:aws:s3vectors:{aws_region}:{aws_account}:bucket/{name}"
    return name, arn
...
class ExtensionStack(Construct):
    def __init__(
        ...
        *,
        ...
        attach_to: Sequence[iam.IRole] | None = None,
        ...
        **kwargs,
        ...
        policy_name = (policy_name_override or str(manifest["policy_name"])).replace(
            "{env}", env_name
        )
        ...
        attached: list[iam.IRole] = [r for r in (attach_to or []) if r is not None]
        ...
        for role_template in manifest.get("attach_policy_to_roles", []):
            ...
            if role is not None and role not in attached:
    ...
    def _provision_indexes_on_platform_bucket(
        ...
        *,
        ...
        for idx_cfg in index_cfgs:
            ...
            # Index ARNs stay stack outputs only. Runtime consumers use the index
            # name; propagating ARNs bloated platform-vars past the SSM 4 KB limit.
